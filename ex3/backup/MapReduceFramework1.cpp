...
{
  auto *vec_to_sort = jc->thread_to_vec[pthread_self ()];
  sort (vec_to_sort->begin (), vec_to_sort->end (), comparator);
  //  print_vec(*vec_to_sort);
}
...
//      for (auto &item: jc->thread_to_vec){
//          IntermediateVec *curr_vec = item.second;
//          if (curr_vec->empty ()){
//              continue;
//            }
//          IntermediatePair curr_pair = curr_vec->back ();
//          if (jc->curr_max_pair == nullptr || jc->curr_max_pair->first < curr_pair.first){
//              jc->curr_max_pair = &curr_pair;
//            }
//        }
...
//              continue;
//            }
//          if (!curr_vec->empty () && pairs_shuffled < num_of_pairs && equals (curr_vec->back (), *(jc->curr_max_pair)))
//          {
//            new_vector->push_back (curr_vec->back ());
//            curr_vec->pop_back ();
//            pairs_shuffled++;
...
JobHandle startMapReduceJob (const MapReduceClient &client,
  ...
  uint64_t done = get_index (counter);
  state->stage = (stage_t) (stage);
  float percentage = min(((float) done / (float) total_size) * 100, (float)100);
